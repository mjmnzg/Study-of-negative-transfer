...
def adaptative_batch_norm(x, 
    ...
    with tf.variable_scope(scope, 'batch_norm', reuse=reuse):
        def training():
            ...
            with tf.name_scope('update_mean'):
                update_m = tf.assign(moving_m, m)
            with tf.name_scope('update_var'):
                update_v = tf.assign(moving_v, v)
